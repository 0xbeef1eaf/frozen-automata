...
class ImageActivity(BaseActivity):
    ...
    def __init__(self, app: "App"):
        ...
        if self.config.image.caption.should() and len(self.pack.caption) > 0:
            # Draw the caption on the canvas (top center), with a transparent background
            self.caption_background = Image.new(
                "RGBA", (self.image.width(), self.image.height() // 4)
            )
            self.caption_background = ImageTk.PhotoImage(self.caption_background)
            self.canvas.create_image(
                0,
                0,
                image=self.caption_background,
                anchor=tkinter.NW,
            )
            self.canvas._caption_background = self.caption_background  # type: ignore
            self.canvas.create_text(
                self.image.width() / 2,
                0,
                anchor=tkinter.N,
                text=random.choice(self.pack.caption),
                font=("Helvetica", 16),
                fill="white",
    ...
    def _on_close_request(self):
        ...
        if self.config.image.mitosis.should():
            for _ in range(self.config.image.mitosis_count.random()):
